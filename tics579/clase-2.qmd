...
La salida de nuestra `Shallow Network` retornará valores reales. Para poder tener una mejor interpretación del significado de cada una aplicaremos la función `Softmax` lo cual permitirá *normalizar* los resultados y llevará los resultados a una ***"distribución de probabilidad"*** (valores positivos que sumen 1).
...
Para medir el error, o la pérdida de información de un punto utilizaremos el Negative Log Loss, también llamado `Cross Entropy Loss`.
...
&= -log \left(\frac{exp(h_{(i = y)}(x))}{\sum_{j=1}^k exp(h_j(x))}\right) \\
...
$$\underset{\theta}{minimize} = \frac{1}{m} \sum_{i=1}^m l(h_\theta(x^{(i)}), y^{(i)}) =\frac{1}{m} \sum_{i=1}^m l(\theta^Tx^{(i)}, y^{(i)})$$
...
## ¿Qué pasa con arquitecturas más complejas?

::: {.columns}
::: {.column}
![](img/clase-1/nn_arq_full.png){.lightbox fig-align="center" width="70%"} 
:::
::: {.column}
::: {.callout-important appearance="default"}
## Próxima Clase
* Vamos a hacer lo mismo para una Red Neuronal.
* La única diferencia es que la hipótesis que utilizaremos es un poco más `fancy`.
:::
:::
::: 


# Nos vemos <br> (Próxima semana no hay clases)
